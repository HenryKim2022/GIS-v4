                                ...
                                <p class="pb-0 mb-0">
                                    @auth
                                        @if (auth()->user()->type == 'admin')
                                            Have a nice day managing this website admin ദ്ദി(｡•̀ ,<)~✩‧₊ @endif
                                            @endauth
                                            @guest
...
@section('footer_page_js')
    ...
    {{-- <script>
        // Categories and institutions data
        const categories = {!! $categories !!};

        // Initialize series data
        const seriesData = categories.map((category) => {
            // Check if the institutions array is not empty
            if (category.institutions.length > 0) {
                return {
                    name: category.name,
                    data: category.institutions.map((institution) => {
                        // Retrieve the total number of transactions for each month
                        return institution.tb_institution.reduce((accumulator, current) => {
                            const transactionMonth = new Date(current.created_at).getMonth();
                            if (!accumulator[transactionMonth]) {
                                accumulator[transactionMonth] = 0;
                            }
                            accumulator[transactionMonth] += 1;
                            return accumulator;
                        }, []);
                    }),
                };
        ...
        // Filter out undefined series data
        const filteredSeriesData = seriesData.filter((series) => series !== undefined);

        // Chart configuration
        const institutionCategoriesChartConfig = {
            chart: {
                height: 218,
                stacked: true,
                type: 'bar',
            ...
            // Add your remaining chart configuration options here
            xaxis: {
        ...
        // Render the chart
        const institutionCategoriesChartEl = document.querySelector('#institutionCategoriesChart');
        if (institutionCategoriesChartEl) {
            const institutionCategoriesChart = new ApexCharts(
                institutionCategoriesChartEl,
                institutionCategoriesChartConfig
            );
            institutionCategoriesChart.render();
        }
    </script> --}}
    ...
    {{-- <script>
        (function() {
            ...
            if (typeof institutionCategoriesChartEl !== undefined && institutionCategoriesChartEl !== null) {
                const institutionCategoriesChart = new ApexCharts(institutionCategoriesChartEl,
                    institutionCategoriesChartConfig);
                institutionCategoriesChart.render();
            }
        })();
